...
# Load party sound
try:
    party_sound = pygame.mixer.Sound("party.wav")
except:
    party_sound = None

def play_party_sound():
    if party_sound:
        party_sound.play()

# Emoji generator based on name
def get_emoji(name):
    emojis = ['🧑‍🎤', '🧒', '👧', '🧑‍🦱', '👨‍🎨', '👩‍🚀', '👨‍🏫', '🧑‍💻', '👸', '🤴', '🎭', '🤹']
    random.seed(hash(name))
    return random.choice(emojis)

# Save file for counts
...
def main():
    ...
    def add_player(name):
        ...
        btn = tk.Button(players_frame, text=f"Party with {name}", command=lambda: [animal.party(), fw_canvas.start_fireworks(), play_party_sound()])
        btn.pack(pady=3)
